...
def get_episodes(ep_id,params={}):
    if ep_id:
        response = generate_request(f"https://integracion-rick-morty-api.herokuapp.com/api/episode/{ep_id}", params)
    else:
    ...
    if response:
        if (ep_id and len(ep_id) == 1):
            for ep in range(len(response)):
                ...
                response_pjs = requests.get(f"https://integracion-rick-morty-api.herokuapp.com/api/character/{list_pj}").json()
...
def get_characters(pj_id, params={}):
    if pj_id:
        response = generate_request(f'https://integracion-rick-morty-api.herokuapp.com/api/character/{pj_id}', params)
    else:
    ...
    if response:
        if pj_id and len(pj_id) == 1:
            ...
            response_ep = requests.get(f"https://integracion-rick-morty-api.herokuapp.com/api/episode/{list_ep}").json()
            response[0]['episodios'] = response_ep

            if (response[0]['origin']['url'] != ""):
                origen_id = int(response[0]['origin']['url'].split('/')[-1])
                res_origen = requests.get(f"https://integracion-rick-morty-api.herokuapp.com/api/location/{[origen_id]}").json()
            ...
            if (response[0]['location']['url'] != ""):
                ...
                res_actual = requests.get(f"https://integracion-rick-morty-api.herokuapp.com/api/location/{[lugar_actual_id]}").json()
...
def get_locations(loc_id, params={}):
    if loc_id:
        response = generate_request(f'https://integracion-rick-morty-api.herokuapp.com/api/location/{loc_id}', params)
    else:
...
def get_info(name, params={}):
    response_pjs = generate_request(f'https://integracion-rick-morty-api.herokuapp.com/api/character/?name={name}', params)
    response_loc = generate_request(f'https://integracion-rick-morty-api.herokuapp.com/api/location/?name={name}', params)
    response_eps = generate_request(f"https://integracion-rick-morty-api.herokuapp.com/api/episode/?name={name}", params)
